  ...
  const sendOTP = async ({ name, username, phone, role, city }) => {
    try {
      ...
      if (!response.ok) {
        return {
          success: false,
          message: data.message || failureMessage(response, 'Failed to send OTP')
        };
      }
  ...
  const login = async (username, password, role) => {
    try {
      ...
      if (!response.ok) {
        return {
          success: false,
          message: data.message || failureMessage(response, 'Login failed')
        };
      }
  ...
  const register = async ({ name, username, phone, password, role, city, otp }) => {
    try {
      ...
      if (!response.ok) {
        return {
          success: false,
          message: data.message || failureMessage(response, 'Registration failed')
        };
      }
